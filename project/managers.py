...
class BraidQueue:
    ...
    def _get_next_level_of_involutions_to_atoms(self, current_level=None):
        # construct dictionary mapping involutions to their sets of atoms, for next length
        if current_level is None:
            return {CoxeterTransform(self.graph): {CoxeterTransform(self.graph)}}
    ...
    def sanity_check(self, upper_length=1000):
        ...
        next_level = self._get_next_level_of_involutions_to_atoms()
        for length in range(max_length + 1):
            ...
            for involution, atoms in next_level.items():
                ...
                if len(self.get_inverse_atoms(self.minimal_relations, word)) < len(atoms):
                    ...
                    return
            self._print('yes')
            next_level = self._get_next_level_of_involutions_to_atoms(next_level)
